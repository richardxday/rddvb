                           ...
                           disableoutput(false)
{
    ...
    static const struct {
        ...
        {"audiosync",                    "aresample=async=1"},
        {"videofilters",                 "-filter:v {conf:videodeinterlace}"},
        {"audiofilters",                 "-filter:a {conf:audiosync}"},
...
{
    return GetConfigItem("videoprobecmd", GetVideoProber() + " \"{filename}\" 2>&1 | grep \"^  Duration:\" | sed -E \"s/^.+([0-9]+):([0-9]+):([0-9\\.]+).+/scale=2; (\\1*3600.0+\\2*60.0+\\3)\\/60.0/\" | bc -l");
}
